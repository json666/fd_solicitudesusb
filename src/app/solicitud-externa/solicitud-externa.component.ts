...
export class SolicitudExternaComponent implements OnInit , OnDestroy{
  ...
  ngOnInit() {
    this.solicitudFrm = this._frmBuilder.group({
      ...
      // 'filtro.fechaHasta': ['', Validators.compose([])],
      // 'filtro.nombreProcedo': ['', Validators.compose([])],
      // 'filtro.documentoIdentidad': ['', Validators.compose([])],
      // 'filtro.nombreSujetoPasivo': ['', Validators.compose([])],
